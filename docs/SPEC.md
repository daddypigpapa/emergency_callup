...
4. 구현이 문서와 달라져야 하면 **문서를 먼저 고치고** 같은 커밋에 코드를 넣는다. 
...
### 2.2 Prototype 교훈 → 설계 결정

| prototype 문제                | 이번 설계의 결정                                                                      | 관련 절    |
| --------------------------- | ------------------------------------------------------------------------------ | ------- |
...
위 모든 watch·타이머는 **전송 루프 객체 1개**가 소유하고 `stop()` 한 곳에서 해제한다
...
## 15. 회귀 테스트 목록 (prototype 검토에서 유래)
...
| 2   | 로그인 ID 체계             | 관리자가 지정(예: 사번)      | 휴대전화번호를 ID로 쓰면 선점 문제가 생김                                                       |
...
## 17. 구현 진행 상황 (2026-09 기준)

이 절은 실제 코딩·실사용자 테스트를 거치며 갱신되는 진행 기록이다. §14 체크리스트와 별도로, "무엇이 실제로 끝났는가"와 "설계에서 벗어난 부분"을 남긴다.

### 17.1 Phase별 상태

| Phase                   | 상태                            | 비고                                                                                                                                                                                     |
| ----------------------- | ----------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Phase 0 (타일 벤치마크)       | 미착수                           | 카카오 vs 브이월드 속도 비교 안 함. 현재는 브이월드(VWorld WMTS)로 고정 운영 중이며, `web/shared/map.js`가 Leaflet 위 얇은 래퍼로 분리되어 있어 추후 교체 시 이 모듈의 함수 시그니처만 유지하면 됨                                                   |
| Phase 1 (데이터 모델·마이그레이션) | 완료                            | SQLite WAL, `internal/store/migrations/*.sql`. 설정 키-값 테이블(`002_settings.sql`) 추가로 확장됨(§17.3 참고)                                                                                        |
| Phase 2 (인증·세션)         | 완료                            | bcrypt + CPU 코어 기준 세마포어(§11.1 M1 대응), 관리자/대원 TTL 분리, 로그인 실패 시 지연 증가(R7)                                                                                                                |
| Phase 3 (명부·임무지역·사건 발령) | 완료                            | xlsx 2단계(미리보기/확정) 가져오기, 휴대전화번호 기준 동일인 판정(R4), §5.4 조별 다수값 자동 채움                                                                                                                        |
| Phase 4 (위치 판정 엔진)      | 완료                            | NOTIFIED→LOGGED_IN/LOC_DENIED→MOVING⇄ARRIVED⇄LEFT 상태기계, 연속 2회 유효 측정 + 최소 시간 간격(도착 10초/이탈 30초)로 GPS 노이즈로 인한 오판정 방지, 체인 기반 속도 이상탐지(직전 유효 측정 대비 55m/s 초과)                                 |
| Phase 5 (현장인력 화면)       | 완료, 실사용자 테스트로 여러 버그 수정(§17.2) | 지도 드래그, 내 위치 표시, 4버튼 하단 고정까지 확인. 실기기 카카오/네이버 딥링크 4가지 조합(§10.3)은 미확인                                                                                                                    |
| Phase 6 (관리자 화면)        | 완료                            | 상황판·조 타일·설정 화면. 다양한 해상도별 스크린샷 비교(§14 Phase 6 항목)는 미실시, 실제 브라우저에서 레이아웃 깨짐은 없음을 확인                                                                                                       |
| Phase 7 (배포)            | 부분 완료                         | Docker Compose + `deploy/key.env`, 빠른 시작 문서화, GitHub 저장소(`daddypigpapa/emergency_callup`) 푸시 완료, cloudflared 임시 터널로 외부 접속 검증 완료. `cmd/sim` 500명/1시간 정식 부하 테스트는 미실시(20명 수준 스모크 테스트만 진행) |

### 17.2 실사용자 테스트로 발견·수정한 실제 버그

개발 중 화면을 직접 켜서 확인하는 과정에서 다음 7가지 문제를 발견하고 수정했다. 설계 문서만으로는 드러나지 않았던, 실제 브라우저/실제 기기 확인이 필요했던 버그들이다.

1. **`/a/`, `/f/` 접속 시 무한 리다이렉트** — 정적 파일 서버가 디렉터리 경로를 index.html로 다시 쓰면서 자기 자신에게 계속 리다이렉트. `internal/httpapi/static.go`에서 불필요한 래퍼 제거로 해결.
2. **지도가 전혀 뜨지 않음** — `leaflet.js` 스크립트 태그 누락(CSS만 링크되어 있었음)으로 전역 `L` 객체가 없어 지도 초기화 자체가 조용히 실패. `<script>` 태그 추가 + 초기화 실패 시 콘솔에 에러를 남기도록 방어 코드 추가.
3. **CSS 수정이 브라우저에 반영되지 않음** — 정적 자산에 1년짜리 캐시 정책이 걸려 있어 수정한 스타일이 계속 옛 버전으로 보임. 캐시 정책을 `no-cache`로 전환(추후 정식 버전 관리 도입 전까지의 임시 조치로 코드에 명시).
4. **`hidden` 속성이 있어도 화면이 겹쳐서 보임** — 일부 CSS 선택자가 `display` 속성을 직접 지정해 브라우저 기본 `[hidden]` 규칙보다 우선순위가 높아지는 바람에, 로그인 화면과 본 화면이 동시에 렌더링됨. `[hidden] { display: none !important; }` 규칙 추가로 해결.
5. **위치 권한을 허용해도 내 위치가 지도에 안 뜸** — 위치는 서버로 전송되고 있었지만, 화면에 점을 찍는 코드(`setMe`) 호출이 애초에 빠져 있었음. `LocationLoop`에 위치 갱신 콜백을 추가해 매 측정마다 지도에 반영하도록 수정.
6. **하단 4개 버튼이 화면 밖으로 사라짐** — 버튼 바가 `position: absolute`로 전체 화면 높이(100dvh) 계산에 의존했는데, 일부 기기에서 실제 화면보다 크게 계산되어 버튼이 화면 밖으로 밀려나고 스크롤도 안 되어 아예 접근 불가능했음. `#mainView`를 CSS grid로 재구성해 버튼 행을 실제 문서 흐름 안에 두는 방식으로 변경, 지도 영역이 남은 공간을 동적으로 채움.
7. **지도를 손가락으로 드래그해도 움직이지 않음** — 페이지 자체 스크롤 방지 설정(`overflow: hidden`)이 일부 기기에서 지도 라이브러리의 터치 제스처까지 가로채는 문제. 지도 영역에 `touch-action: none`을 명시적으로 지정해 해결(실제 터치 기기에서의 최종 확인은 이 환경의 한계로 미실시, 코드/CSS 레벨 방어 조치까지만 검증).

### 17.3 원래 설계에서 벗어난/추가된 부분 (§0 규칙 4에 따른 기록)

- **색상 전면 제거**: §8 등에서 색상으로 구분하던 상태 표시를 전부 선의 굵기·스타일(실선/점선/이중선)과 텍스트로 대체. 사용자 요청("색을 넣지 말고 선으로만")에 따른 디자인 방향 변경.
- **관리자가 지도 인증키를 화면에서 직접 설정**: 원래 설계에는 없던 기능. 배포 환경마다 다른 VWorld 키를 코드 수정·재배포 없이 바꿀 수 있도록 설정 테이블(`setting` 키-값 테이블, `internal/settings`)과 관리자 전용 API(`GET/PUT /api/v1/a/settings/tile-key`)를 추가.
- **현장 화면 지도 포커스 버튼 3단계 동작**: §8.1.3의 단순 토글 대신, [내 위치]는 내 위치를 중앙에, [목적지]는 임무지역을 중앙에 각각 독립적으로 포커싱하고, 같은 버튼을 한 번 더 누르면 "내 위치+임무지역 둘 다 보기"로 돌아오는 3단계(`both`/`me`/`area`) 방식으로 구현. 사용자가 실사용 흐름을 시험해보며 구체화한 요구사항.
- **`deploy/.env` → `deploy/key.env` 이름 변경**: 실제 인증키가 담긴 파일임을 이름에서 더 분명히 하기 위해 변경(`.gitignore`도 `*.env` 패턴으로 조정).

### 17.4 진행 예정

- **임무지역·조별 계획 사전 설정 화면** — 격자(칸) 단위 임무지역 편집(클릭·드래그·행정동 일괄 선택), 조별 집결지·체크포인트, 발령 자동 채움 연동. 구현명세: [SPEC_AREA_EDITOR.md](SPEC_AREA_EDITOR.md). 완료 시 §4·§5.4·§6.2·§16-4를 그 문서 §9에 따라 본문에 반영한다.

### 17.5 아직 검증되지 않은 항목

- Phase 0 카카오 vs 브이월드 타일 벤치마크
- §10.3 카카오맵/네이버지도 딥링크의 실기기(안드로이드/iOS × 앱 설치/미설치) 4가지 조합 확인
- `cmd/sim` 500명/1시간 정식 규모 시뮬레이션(현재는 20명 수준 스모크 테스트만 실시)
- 실제 터치스크린 기기에서의 지도 드래그 제스처 최종 확인(코드 레벨 수정과 CSS 계산상 검증까지만 완료; 이 개발 환경은 실제 터치 이벤트 시뮬레이션을 지원하지 않아 브라우저 자동화 도구로는 확인 한계가 있었음이 확인됨)

---

## 18. 장래 구현 목표 (일정 미정 · 착수 전 사용자 승인 필요)

### 18.1 내장 주소 검색 엔진

**목표**: 집결지·체크포인트·임무지역 위치를 입력할 때, 현장에서 실제로 쓰는 여러 위치 표기를 **외부 API 없이** 서버 안에서 즉시 좌표로 바꾼다. 지금은 도로명 기초번호만 VWorld 지오코더(§SPEC_AREA_EDITOR §4.3)에 의존하며, 통신 장애·키 문제·응답 지연이 그대로 입력 지연이 된다.

**지원할 표기**

| 표기 | 예 | 좌표 확정 방법 | 데이터 출처(예정) |
|---|---|---|---|
| 국가지점번호 | `다사 1234 5678` | DB 불필요 — UTM-K 격자 좌표를 수식으로 환산 | 행정안전부 고시 산식 |
| 도로명주소 | `대구광역시 중구 공평로 88`, `공평로 88` | 도로명·건물번호 → 건물 대표점 | 도로명주소 전체자료(juso.go.kr, 월 1회 갱신) |
| 지자체 기초번호(기초번호판) | `공평로 88` (건물 없는 지점) | 도로구간 + 기초번호 → 도로 중심선 위 지점 | 도로명주소 기초번호 자료(juso.go.kr) |
| 지자체 가로등 번호 | `중구-가로등-0123` (지자체별 형식 상이) | 관리번호 → 설치 좌표 | 전국가로등표준데이터(data.go.kr) + 해당 지자체 제공 파일 |

**설계 원칙**

1. **표기별 정규화 → 단일 검색 키**. 입력 문자열을 표기 종류로 먼저 분류(정규식·접두 규칙)하고, 종류마다 정규화 함수를 둔다(공백·하이픈·괄호 제거, 전각→반각, `번길`·`로` 표기 통일, 지자체 접두어 보정). 정규화된 키로 한 번에 찾는다. 사용자 입력 형식이 조금씩 달라도 같은 키가 나오는 것이 목적이다.
2. **SQLite 안에 둔다.** 별도 검색 서버 금지(§2.1). `addr_*` 테이블 + 정규화 키 인덱스(접두 검색용) + 필요 시 SQLite FTS5. 데이터 크기를 줄이기 위해 **기관 관할 시·군·구만 적재**하고, 관할은 설정 화면에서 고른다.
3. **적재는 관리자 화면에서 파일 업로드로.** 공공데이터 파일(zip/txt/csv)을 그대로 올리면 서버가 파싱·정규화·색인한다(§7.3 xlsx 가져오기와 같은 2단계 미리보기/확정 방식). 갱신 주기·마지막 적재일을 화면에 표시한다.
4. **로컬 우선, 외부 보조.** 검색 순서: 내장 DB → (없으면) VWorld 지오코더. 어느 쪽에서 찾았는지 결과에 표시한다. 국가지점번호는 항상 로컬 수식만 쓴다.
5. **응답 목표**: 관할 시·군·구 1개 기준 검색 1건 50 ms 이내, 자동완성(접두 검색) 20 ms 이내. 500명이 동시에 쓰는 화면이 아니라 관리자 수 명이 쓰는 화면이므로 이 정도면 충분하다.
6. **기존 위치 입력 위젯(§SPEC_AREA_EDITOR §5.3)의 뒤만 바꾼다.** 화면은 그대로, `GET /a/geo/geocode`가 내장 엔진을 먼저 타도록 한다. 표기 종류 자동 판별이 실패하면 지금처럼 안내 문구를 낸다.

**하지 않는 것(1차 범위 밖)**: 지번주소(구주소)·건물명 검색, 전국 단위 적재, 지도 위 역검색(좌표 → 주소), 실시간 공공데이터 API 연동.

**착수 전 확인할 것**
- 가로등 번호 자료의 실제 형식(지자체마다 다름) — 관할 지자체 파일 샘플 1개 확보 후 정규화 규칙을 정한다.
- 도로명주소 전체자료 라이선스·재배포 조건, 관할 1개 시·군·구 적재 시 DB 파일 증가량(예상 수십 MB).
- 국가지점번호 산식의 기준 원점·격자 상수는 고시 원문으로 검증한다(§10 범위 밖으로 두었던 §SPEC_AREA_EDITOR 항목을 이때 함께 해소).
